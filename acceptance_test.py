...
def test_ping() -> None:
    ...
    assert_eq(store.handle_line("PING"), "OK PONG")
...
def test_malformed_commands() -> None:
    ...
    assert_eq(store.handle_line("SET_REQ clientA:0 onlykey"), "ERR usage: SET_REQ <request_id> <key> <value...>")
    assert_eq(store.handle_line("CAS_REQ clientA:0 k nope value"), "ERR bad_version")
    assert_eq(store.handle_line("SET_REQ badrequestid k value"), "ERR invalid_request_id")
